...
class PartnersController extends Controller
{
     ...
     if ($request->ajax()) {
        ...
        return Datatables::of($data)
                ...
                ->addColumn('logo', function ($data) { 
                     ...
                     $url=asset("/storage_upload/$data->logo"); 
          ...
          return view('dashborad.partners.view') ; 
   ...
   public function store(PartnerRequest $request)
   {
    ...
    Storage::disk('public')->put($path.$name_image, file_get_contents( $logo ));
  
    $partner->logo =$name_image ;
   ...
   {
    $result =  Partner::find($id)->delete($id);
   ...
   public function update(Request $request)
   {
      ...
      $logo = $request->file('logo') ;
      if($request['name'] != null ) $result->name = $request['name'] ; 
      if(isset($logo))
      {
        $path = 'uploads/images/';
        $name_image = time()+rand(1,10000000).'.'.$logo->getClientOriginalExtension();
