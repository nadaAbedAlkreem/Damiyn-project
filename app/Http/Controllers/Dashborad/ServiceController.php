...
class ServiceController extends Controller
{
      ...
      if ($request->ajax()) {
         ...
         return Datatables::of($data)
                 ...
                 ->addColumn('action', function ($data) {
                   return '
                   <button type="button"  class="btn btn-xs btn-primary"    data-header="'.$data->header.'"  data-body="'.$data->body.'"   data-id="'.$data->id.'"         
                 ...
                 ->addColumn('icon', function ($data) { 
                      ...
                      $url=asset("/storage_upload/$data->icon"); 
           ...
           return view('dashborad.services.view') ; 
    ...
    public function store(ServiceRequest $request)
    {
     ...
     $path = 'uploads/images/';
     $name_image = time()+rand(1,10000000).'.'.$icon->getClientOriginalExtension();
     Storage::disk('public')->put($path.$name_image, file_get_contents( $icon ));
   
     $services->icon =$name_image ;
    ...
    {
     $result =  Services::find($id)->delete($id);
    ...
    public function update(Request $request)
    {
       ...
       $icon = $request->file('icon') ;
       if($request['header'] != null ) $result->header = $request['header'] ; 
       if($request['body'] != null ) $result->body = $request['body'] ; 
